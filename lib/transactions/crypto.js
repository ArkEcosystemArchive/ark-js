...
function fromBytes(hexString){
	...
	if(tx.type == 0){ // transfer
		tx.recipientId = bs58check.encode(new Buffer(hexString.substring(76,76+42),"hex"));
		tx.vendorFieldHex = hexString.substring(76+42,76+42+128);
		tx.amount = new Buffer(hexString.substring(76+42+128,76+42+128+16),"hex").readUInt32LE(0);
		tx.fee = new Buffer(hexString.substring(76+42+128+16,76+42+128+32),"hex").readUInt32LE(0);
		tx.signature = hexString.substring(76+42+128+32);
		if(tx.signature.length == 0) delete tx.signature;
		else {
			var length = "0x"+tx.signature.substring(2,4);
			length = parseInt(length, 16)+2;
			tx.signature = hexString.substring(76+42+128+32,76+42+128+32 + length*2);
			tx.signSignature = hexString.substring(76+42+128+32+length*2);
		...
		tx.signature = hexString.substring(76+42+128+32+66);
		if(tx.signature.length == 0) delete tx.signature;
		else {
			var length = "0x"+tx.signature.substring(2,4);
			length = parseInt(length, 16)+2;
			tx.signature = hexString.substring(76+42+128+32+66,76+42+128+32+66 + length*2);
			tx.signSignature = hexString.substring(76+42+128+32+66+length*2);
			if(tx.signSignature.length == 0) delete tx.signSignature;
		}
	}
		...
		var signature1 = hexString.substring(hexString.length-145);
		var signature2 = null;
		var offset = 0;
		var found = false;
		while(!found && signature1.length > 8){
			try {
				ECSignature.fromDER(new Buffer(signature1,"hex"));
				found = true;
			} catch(error){
				signature1 = signature1.substring(1);
			}
		}
		if(!found){
			signature1 = null;
			...
			found = false;
			offset = signature1.length;
			var signature2 = hexString.substring(hexString.length-145-offset, hexString.length-offset);
			while(!found && signature2.length > 8){
				try {
					ECSignature.fromDER(new Buffer(signature2,"hex"));
					found = true;
				} catch(error){
					signature2 = signature2.substring(1);
				}
			}
			if(!found){
				signature2 = null;
				...
				tx.signSignature = signature1;
				tx.signature = signature2;
				offset += signature2.length
			}
			else {
				tx.signature = signature1;
			}
		}
